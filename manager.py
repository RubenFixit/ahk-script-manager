...
def generate_loader(data_dir: Path, catalog: dict[str, Any], validation: bool = False) -> Path:
    ...
    lines.extend([
        ...
        "    ASM_ApplyRemoteMode(true, \"Manual Remote Mode changed\")",
        "}",
        ...
        "        ASM_ApplyRemoteMode(true, isRemote ? \"Remote client detected\" : \"Remote client no longer active\")",
        "    }",
        "}",
        "ASM_ApplyRemoteMode(showNotice := false, reason := \"\") {",
        ...
        "    }",
        "    if showNotice {",
        "        status := shouldSuspend ? \"Managed hotkeys paused\" : \"Managed hotkeys active\"",
        "        TrayTip(reason = \"\" ? status : reason \" - \" status, \"AutoHotkey Script Manager\")",
        "    }",
        "}",
    ])
    if remote_processes:
